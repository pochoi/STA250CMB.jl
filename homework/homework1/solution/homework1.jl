...
calculateΣ(cov::Vector{Cov}, x_obs) = [ calculateΣ(cov[k], x_obs) for k in eachindex(cov)]

function loglikelihood(Σ::Matrix{Float64}, x)
    k = size(Σ, 1)
    return - 0.5 * dot(x, Σ \ x) - 0.5 * log((2π)^k * det(Σ))
end
...
mesh_side = 100
nsim = 1
xmesh, ymesh, fx2d_pre = grf2d(Cov1(), mesh_side, x2d_obs, fx2d_obs, nsim)
spyplot(xmesh[:], ymesh[:], fx2d_pre[1], x2d_obs, fx2d_obs)

xmesh, ymesh, fx2d_pre = grf2d(Cov2(), mesh_side, x2d_obs, fx2d_obs, nsim)
spyplot(xmesh[:], ymesh[:], fx2d_pre[1], x2d_obs, fx2d_obs)

xmesh, ymesh, fx2d_pre = grf2d(Cov3(), mesh_side, x2d_obs, fx2d_obs, nsim)
spyplot(xmesh[:], ymesh[:], fx2d_pre[1], x2d_obs, fx2d_obs)
